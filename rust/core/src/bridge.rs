...
impl ResponseItem {
    ...
    ///
    /// Keeps `"0x..."` hex format for nullifiers (not `"nil_..."`/`"snil_..."` canonical format).
    fn from_protocol_item(
...
pub struct CachedSignalHashes {
    ...
    pub(crate) legacy_signal_hash: String,
}
...
/// # Errors
///
/// Returns an error if the request kind is not a uniqueness proof.
...
mod tests {
    ...
    fn test_bridge_request_payload_serialization() {
        ...
        let signature = alloy_primitives::Signature::from_str(&sig_65_bytes).unwrap();
